...
export function floorHandler(socket: Socket, io: Server): void {
  ...
  socket.on('floor:request-status-update', (data: {
    ...
    logger.info(
      `floorHandler`,
      `Socket ${socket.id} requested table change ${data.tableId} → ${data.status}. Use REST API PATCH /api/tables/:id.`
    );
  ...
  socket.on('floor:request-full-sync', () => {
    logger.info('floorHandler', `Socket ${socket.id} requested full sync — use GET /api/tables`);
    socket.emit('floor:request-acknowledged', {
