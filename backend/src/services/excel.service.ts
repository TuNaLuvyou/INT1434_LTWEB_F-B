...
export class ExcelService {
  async generateRevenueReport(res: Response, from: Date, to: Date, type: 'full' | 'summary', tenantId: string, branchId?: string): Promise<void> {
    console.time('[Excel] generate');
    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'HiAI-MenuGo System';
    workbook.created = new Date();

    const baseWhere: any = { paidAt: { gte: from, lte: to }, tenantId };
    if (branchId) baseWhere.branchId = branchId;

    await this.buildRevenueSummarySheet(workbook, from, to, baseWhere);

    if (type === 'full') {
      await this.buildOrderDetailsSheet(workbook, from, to, baseWhere);
      await this.buildBestSellersSheet(workbook, from, to, tenantId, branchId);
      const shiftWhere: any = { closedAt: { gte: from, lte: to }, tenantId };
      if (branchId) shiftWhere.branchId = branchId;
      await this.buildShiftSummarySheet(workbook, from, to, shiftWhere);
    }
  ...
  private async buildRevenueSummarySheet(wb: ExcelJS.Workbook, from: Date, to: Date, baseWhere: any): Promise<void> {
    ...
    const dailyData = await getRevenue(from.toISOString(), to.toISOString(), 'day', baseWhere.tenantId, baseWhere.branchId);
  ...
  private async buildOrderDetailsSheet(wb: ExcelJS.Workbook, from: Date, to: Date, baseWhere: any): Promise<void> {
    ...
    for (const p of payments) {
      ...
      const row = sheet.addRow({
        id: p.id,
        table: p.session.table ? p.session.table.label : 'Mang về',
        time: p.paidAt,
        shift: p.shiftId,
  ...
  private async buildBestSellersSheet(wb: ExcelJS.Workbook, from: Date, to: Date, tenantId: string, branchId?: string): Promise<void> {
  ...
  private async buildShiftSummarySheet(wb: ExcelJS.Workbook, from: Date, to: Date, shiftWhere: any): Promise<void> {
