...
export async function voidOrderItem(req: Request, res: Response): Promise<void> {
  try {
    ...
    try {
      await ingredientService.reverseInventory(orderItemId, voidedBy, authReq.user?.tenantId, authReq.user?.branchId);
    ...
    emitKitchenItemUpdated(voidedItem.session.tenantId, voidedItem.session.branchId, {
      sessionId,
      orderItemId,
      tableId,
      menuItemName: voidedItem.menuItem.name,
    ...
    emitCartUpdated(voidedItem.session.tenantId, voidedItem.session.branchId, tableId, {
      sessionId,
      tableId,
