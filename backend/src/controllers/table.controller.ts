...
export const handleCreateTable = async (req: Request, res: Response) => {
  try {
    ...
    const authReq = req as AuthenticatedRequest;
    const tenantId = authReq.user?.tenantId;
    let branchId = authReq.user?.branchId;
    if (!branchId) {
      branchId = req.body.branchId;
      if (branchId) {
        const branch = await prisma.branch.findUnique({ where: { id: branchId } });
        if (!branch || branch.tenantId !== tenantId) {
          return res.status(403).json({ success: false, message: 'Chi nhánh không hợp lệ.' });
        }
      }
    }
    ...
    if (!branchId) {
      return res.status(400).json({ success: false, message: 'Không xác định được chi nhánh. Vui lòng chọn chi nhánh trước khi tạo bàn.' });
    }
