...
export async function updateKdsItemStatus(req: Request, res: Response): Promise<void> {
  try {
    ...
    emitKitchenItemUpdated(updatedItem.tenantId, updatedItem.session.branchId, {
      orderItemId: updatedItem.id,
      sessionId: updatedItem.sessionId,
      removedOrderItemId: statusUpdate.removedOrderItemId,
      tableId: updatedItem.session.tableId || updatedItem.sessionId,
    ...
    if (updatedItem.session.tableId) {
    ...
    if (status === 'DONE') {
      ...
      if (allDone) {
        emitSessionAllDone(updatedItem.tenantId, updatedItem.session.branchId, {
          sessionId: updatedItem.sessionId,
          tableId: updatedItem.session.tableId || updatedItem.sessionId,
          tableNumber: updatedItem.session.table?.tableNumber ?? 0,
          label: updatedItem.session.table?.label ?? 'Mang về'
        });
      }
    }
...
export async function voidKdsOrderItem(req: Request, res: Response): Promise<void> {
  try {
    ...
    const tableId = voidedItem.session.tableId || voidedItem.sessionId;
    ...
    if (voidedItem.session.tableId) {
      emitOrderStatusChanged(voidedItem.session.tableId, {
        orderItemId,
        menuItemId: voidedItem.menuItemId,
        sessionId,
        tableId: voidedItem.session.tableId,
        status: 'VOID',
    ...
    emitKitchenItemUpdated(voidedItem.tenantId, voidedItem.session.branchId, {
      orderItemId,
      sessionId,
      tableId,
      menuItemName: voidedItem.menuItem.name,
    ...
    emitCartUpdated(voidedItem.tenantId, voidedItem.session.branchId, tableId, {
      sessionId,
      tableId,
