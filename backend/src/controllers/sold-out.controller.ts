...
export const updateSoldOut = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    // 1. Kiểm tra món ăn tồn tại và thuộc tenant của người dùng (chống IDOR cross-tenant)
    ...
    logger.info('Socket.io', `Sold-out events emitted → menuItemId=${id}, name=${updated.name}, isSoldOut=${isSoldOut}`);
    ...
    await triggerNextRevalidation().catch((err) => {
      ...
      logger.warn('Revalidate', `Could not invalidate Next.js cache: ${err.message}`);
    });
    ...
    logger.error('SoldOut', 'Failed to update isSoldOut:', error);
...
async function triggerNextRevalidation(): Promise<void> {
  ...
  if (!secret) {
    throw new Error('REVALIDATION_SECRET chưa được cấu hình trong environment');
  }
  ...
  logger.info('Revalidate', 'SSG cache /table/[tableId] invalidated successfully');
}
