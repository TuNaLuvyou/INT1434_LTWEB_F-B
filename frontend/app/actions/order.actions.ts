...
export async function submitOrder(
  ...
  try {
    ...
    // emitCashierNewOrder và emitCartUpdatedAfterSubmit sẽ gracefully fail (warn only) nếu socket chưa init.
    try {
      const { emitCashierNewOrder, emitCartUpdatedAfterSubmit } = await import(
...
export type RequestPaymentResult =
  | { success: true; method: 'COUNTER'; total: number; message: string }
  | { success: true; method: 'VIETQR'; data: VietQRData }
  | { success: false; message: string };

/**
 * Server Action — Khách chọn phương thức thanh toán từ trang QR Menu.
 * Gọi backend POST /api/sessions/:sessionId/request-payment
 */
export async function requestPayment(input: {
  ...
  try {
    const res = await fetch(`${API_URL}/api/sessions/${input.sessionId}/request-payment`, {
      method: 'POST',
    ...
    if (json.method === 'COUNTER') {
      return { success: true, method: 'COUNTER', total: json.total, message: json.message };
    }
    ...
    return { success: false, message: 'Mất kết nối mạng. Vui lòng thử lại.' };
  }
}

/**
 * Server Action — Khách hủy yêu cầu VietQR (chuyển sang trả tại quầy).
 * Gọi backend POST /api/sessions/:sessionId/cancel-payment
 */
export async function cancelPaymentRequest(input: {
  ...
  try {
    const res = await fetch(`${API_URL}/api/sessions/${input.sessionId}/cancel-payment`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      cache: 'no-store',
    });
    ...
    return { success: false, message: 'Mất kết nối mạng.' };
  }
}
