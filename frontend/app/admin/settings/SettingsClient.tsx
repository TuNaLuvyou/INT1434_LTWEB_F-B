...
import { RefreshCw, MapPin, Compass, Loader2, Save, Info, Shield, CheckCircle2, CreditCard, Store, Edit3, X, Check, Palette, Award, Settings } from 'lucide-react';
...
export default function SettingsClient() {
  ...
  const [activeTab, setActiveTab] = useState<'geofence' | 'sync' | 'info' | 'branches' | 'branding'>('geofence');
  ...
  return (
    <div className="h-screen bg-zinc-950 text-zinc-50 flex flex-col font-sans relative overflow-hidden">
      ...
      {/* Header (Standardized across Admin Suite) */}
      <AdminHeader
        title="Cài Đặt Hệ Thống"
        icon={<Settings size={13} className="stroke-[2.5]" />}
        rightSide={
      ...
      <main className="flex-1 min-h-0 overflow-hidden flex flex-col p-3 sm:p-6 max-w-7xl w-full mx-auto relative z-10">
        ...
        <AdminTabs
          items={[
            { key: 'geofence', label: t('geofencingTab'), icon: <MapPin className="h-4 w-4 shrink-0" /> },
            { key: 'sync', label: t('syncTab'), icon: <RefreshCw className="h-4 w-4 shrink-0" /> },
            { key: 'info', label: t('infoTab'), icon: <Info className="h-4 w-4 shrink-0" /> },
            { key: 'branches', label: t('branchesTab'), icon: <Store className="h-4 w-4 shrink-0" /> },
            { key: 'branding', label: t('brandingTab'), icon: <Palette className="h-4 w-4 shrink-0" /> },
          ]}
          activeKey={activeTab}
          onChange={(key) => setActiveTab(key as 'geofence' | 'sync' | 'info' | 'branches' | 'branding')}
          className="mb-5 shrink-0"
        />

        {/* Tab Content Container */}
        <div className="flex-1 min-h-0 overflow-y-auto py-2 scrollbar-auto-hide">
          {activeTab === 'geofence' && (
            <form onSubmit={handleSaveConfig} className="space-y-6 w-full">
