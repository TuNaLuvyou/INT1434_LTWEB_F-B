  ...
  return (
    <div className="h-screen bg-zinc-950 text-zinc-50 flex flex-col font-sans relative overflow-hidden">
      ...
      <AdminHeader
        ...
        icon={<Ticket size={13} className="stroke-[2.5]" />}
        rightSide={
      ...
      <main className="flex-1 min-h-0 overflow-hidden flex flex-col p-3 sm:p-6 max-w-7xl w-full mx-auto relative z-10">
        <AdminTabs
          items={[
            { key: "vouchers", label: "Mã Giảm Giá (Vouchers)", icon: <Ticket className="h-3.5 w-3.5" /> },
            { key: "tiers", label: "Hạng Thành Viên", icon: <Award className="h-3.5 w-3.5" /> },
            { key: "members", label: "Danh Sách Thành Viên", icon: <Users className="h-3.5 w-3.5" /> },
          ]}
          activeKey={activeTab}
          onChange={(key) => setActiveTab(key as 'vouchers' | 'tiers' | 'members')}
          className="mb-4"
        />
            ...
            <div className="bg-zinc-900/40 border border-zinc-900 rounded-3xl p-4 sm:p-5 space-y-4 shrink-0">
              ...
              <form onSubmit={handleCreateVoucher} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full h-11 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(99,102,241,0.2)] disabled:opacity-50 cursor-pointer"
                >
                  {actionLoading ? (
                          ...
                          <tr key={v.id} className="hover:bg-zinc-900/10 transition-all">
                            ...
                            <td className="px-4 py-3.5 text-center">
                              <button
                                ...
                                disabled={actionLoading}
                                className="p-1.5 rounded-lg border border-zinc-900 text-zinc-500 hover:text-rose-400 hover:border-rose-500/30 transition-all cursor-pointer"
        ...
        {/* Tab 2: Membership Tier Config */}
        {activeTab === 'tiers' && (
          <MembershipTab />
        )}

        {/* Tab 3: Member List & Points Management */}
        {activeTab === 'members' && (
          <MemberListTab />
        )}
