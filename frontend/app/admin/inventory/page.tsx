...
export default function AdminInventoryPage() {
  ...
  return (
    <div className="h-screen bg-zinc-950 text-zinc-50 flex flex-col font-sans relative overflow-hidden">
      ...
      {/* Header */}
      <AdminHeader
        title="Quản Lý Nguyên Liệu & Kho Hàng"
        icon={<Warehouse size={13} className="stroke-[2.5]" />}
        rightSide={
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-zinc-800 bg-zinc-900/40 text-zinc-300 text-xs font-medium">
              <Calendar className="h-3.5 w-3.5 text-zinc-500" />
              <span>{today}</span>
            </div>
            <button onClick={handleRefresh} className="h-9 w-9 rounded-xl border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-900 transition-all cursor-pointer">
      ...
      <main className="flex-1 min-h-0 overflow-hidden flex flex-col p-3 sm:p-6 max-w-7xl w-full mx-auto relative">
        ...
        {activeTab === "main" && (
          <div className="flex-1 min-h-0 overflow-hidden bg-zinc-900/40 border border-zinc-900 rounded-3xl p-5 flex flex-col space-y-4">
            {loading ? (
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between shrink-0">
              ...
              <div className="flex gap-2 items-center">
                ...
                <button onClick={() => setShowAddModal(true)} className="flex items-center gap-1.5 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all shadow-[0_0_15px_rgba(124,58,237,0.3)] shrink-0 cursor-pointer">
