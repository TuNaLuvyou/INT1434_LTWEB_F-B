          ...
          <div className="w-full max-w-5xl h-[92vh] sm:h-[88vh] bg-zinc-950 border border-zinc-800/90 rounded-3xl shadow-2xl flex flex-col overflow-hidden text-zinc-100 relative">
            ...
            <div className="px-6 py-4 border-b border-zinc-800/80 bg-zinc-950 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-zinc-50">Thêm Bàn Ăn Mới</h3>
                  <p className="text-xs text-zinc-400">Thiết lập bàn ăn mới vào sơ đồ phục vụ và tự động tạo mã QR</p>
                </div>
              </div>
              <button 
                onClick={() => {
            ...
            {/* 2-Column Form Body */}
            <form onSubmit={handleAddTable} className="flex-1 flex flex-col min-h-0 overflow-hidden">
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                
                {addError && (
                  <div className="p-3.5 bg-red-950/30 border border-red-500/30 rounded-2xl text-red-400 text-xs font-bold flex items-center gap-2">
                    <AlertCircle size={16} />
                    <span>{addError}</span>
                  </div>
                )}
                  ...
                  <div className="space-y-5 bg-zinc-900/40 border border-zinc-800/60 p-5 rounded-3xl">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        disabled={addLoading}
                        className="w-full bg-zinc-950 border border-zinc-800 focus:border-emerald-500 rounded-2xl px-4 py-3 text-sm font-semibold text-zinc-100 focus:outline-none transition-all placeholder-zinc-600"
                      />
                      <p className="text-[11px] text-zinc-500">Dùng để sắp xếp và định danh duy nhất cho bàn.</p>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        type="text"
                        placeholder="Ví dụ: Bàn Cửa Sổ 1, Bàn VIP 2..."
                        value={newLabel}
                        onChange={(e) => setNewLabel(e.target.value)}
                        disabled={addLoading}
                        className="w-full bg-zinc-950 border border-zinc-800 focus:border-emerald-500 rounded-2xl px-4 py-3 text-sm font-semibold text-zinc-100 focus:outline-none transition-all placeholder-zinc-600"
                      />
                      <p className="text-[11px] text-zinc-500">Tên bàn hiển thị trực quan trên giao diện POS & Khách đặt món.</p>
                    </div>
                  </div>

                  {/* RIGHT COLUMN: Live Table Card Preview */}
                  <div className="space-y-5 flex flex-col justify-between">
                    <div className="space-y-4 bg-zinc-900/40 border border-zinc-800/60 p-5 rounded-3xl">
                      <h4 className="font-extrabold text-zinc-100 text-sm border-b border-zinc-800 pb-2 flex items-center gap-2">
                        <TableIcon size={16} className="text-emerald-400" />
                        <span>Xem trước thẻ bàn ăn (Preview)</span>
                      </h4>

                      {/* Card Preview */}
                      <div className="bg-zinc-950 border border-emerald-500/30 p-6 rounded-3xl space-y-4 shadow-lg shadow-emerald-500/5">
                        <div className="flex items-center justify-between">
                          <span className="px-3 py-1 bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 text-xs font-black rounded-xl">
                            BÀN SỐ #{newTableNumber || "??"}
                          </span>
                          <span className="text-[10px] font-bold bg-zinc-900 text-zinc-400 px-2.5 py-1 rounded-lg border border-zinc-800">
                            TRẠNG THÁI: TRỐNG
                          </span>
                        </div>

                        <div>
                          <h5 className="text-base font-black text-zinc-100">
                            {newLabel.trim() || `Bàn số ${newTableNumber || "??"}`}
                          </h5>
                          <p className="text-xs text-zinc-400 font-semibold mt-0.5">Sẵn sàng đón khách & nhận order QR</p>
                        </div>

                        <div className="pt-3 border-t border-zinc-900 flex items-center justify-between text-xs font-bold text-zinc-400">
                          <span>Mã QR Order:</span>
                    ...
                    <div className="p-4 bg-zinc-950 border border-zinc-800 rounded-3xl text-center">
                      <p className="text-xs text-zinc-400 font-semibold">Sau khi tạo bàn, bạn có thể xuất mã QR Code cho khách tự gọi món.</p>
                    </div>
                  </div>
              ...
              <div className="px-6 py-4 border-t border-zinc-800/80 bg-zinc-950 flex items-center justify-end gap-3 shrink-0">
                <button
                  ...
                  disabled={addLoading}
                  className="px-6 py-3 border border-zinc-800 text-zinc-400 rounded-2xl text-xs font-bold hover:bg-zinc-900 hover:text-zinc-200 transition-all cursor-pointer disabled:opacity-50"
                >
                  Hủy bỏ
                </button>
                <button
                  type="submit"
                  disabled={addLoading}
                  className="px-8 py-3 bg-gradient-to-r from-emerald-600 to-teal-500 text-white rounded-2xl text-xs font-black hover:from-emerald-500 hover:to-teal-400 shadow-xl shadow-emerald-600/20 disabled:opacity-50 transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
                  {addLoading ? (
                    <>
                    ...
                    <>
                      <Plus className="h-4 w-4" /> Tạo Bàn Ăn Mới
                    </>
                  )}
                </button>
              </div>
      ...
      {qrTableId && tables.find(t => t.id === qrTableId) && (
        ...
        >
          <div className="relative animate-scale-in" onClick={e => e.stopPropagation()}>
            ...
            <TableQRCode table={tables.find(t => t.id === qrTableId)!} tenantId={user?.currentTenantId ?? ''} branchId={user?.currentBranchId ?? ''} />
          </div>
        </div>
      )}
    </div>
