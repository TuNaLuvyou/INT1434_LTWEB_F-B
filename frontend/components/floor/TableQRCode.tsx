...
export default function TableQRCode({ table }: TableQRCodeProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-6 rounded-3xl bg-zinc-950/60 border border-zinc-800 backdrop-blur-sm relative overflow-hidden group">
      ...
      {/* Container hiển thị mã QR */}
      <div id={`print-qr-${table.id}`} className="flex flex-col items-center bg-white p-6 rounded-2xl shadow-sm">
      ...
      <p className="text-[10px] text-zinc-500 text-center select-all select-none font-mono break-all opacity-60">
        {menuUrl}
      </p>
    </div>
  );
