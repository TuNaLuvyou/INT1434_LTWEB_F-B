      ...
      <div className="px-6 py-4 border-b border-zinc-800/80 bg-zinc-950 flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-lg font-black text-zinc-50">
            Công thức Định lượng: <span className="text-violet-400">{menuItemName}</span>
          </h2>
          <p className="text-xs text-zinc-400 font-semibold">BOM — Thiết lập nguyên liệu cho 1 suất bán</p>
        </div>
        {onClose && (
          <button onClick={onClose} className="p-2.5 rounded-2xl text-zinc-400 hover:text-white hover:bg-zinc-800 border border-zinc-800/80 transition-all cursor-pointer">✕</button>
        )}
      </div>
